...
const CurrentGauge = ({ value, status }: CurrentGaugeProps) => {
  ...
  // Calculate percentage (0-1.6A range)
  const percentage = Math.min((animatedValue / 1.6) * 100, 100);
  const angle = (percentage / 100) * 270 - 135; // -135 to 135 degrees
  
  const getColor = () => {
    if (value > 1.5) return '#ff4444';
    if (value > 1.2) return '#ffaa00';
    if (value < 0.01) return '#ff4444';
    return '#00dd00';
  };
  ...
  return (
    ...
    >
      <div className="gauge-header depth-layer-1">
        <Zap size={24} color="var(--primary-color, #667eea)" className="float-animation" />
        <h2>Current</h2>
      </div>
      ...
      <div className="circular-gauge-container">
        ...
        <div className="gauge-scale">
          ...
          <span className="scale-mark mid">0.8</span>
          <span className="scale-mark end">1.6</span>
        </div>
      </div>
      ...
      <div className="gauge-info">
        <div className="gauge-range-display">
          <div className="range-item">
            ...
            <span className="range-value">0.01-1.2A</span>
          </div>
